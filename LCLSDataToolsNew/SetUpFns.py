...
def IscatFilters(paramDict,outDict):
    ...
    f_Iscat1=(Iscat>Iscat_thresh)
    print('Iscat_thresh keeps ', 
          np.nonzero(f_Iscat1)[0].shape[0]/outDict['numshots'])
    
        ##create filter on xray intensity keeping 80% of shots
    l,r,frac,f_Iscat=slice_histogram(Iscat,f_xon&(f_Iscat1),0.99, 
                                      showplot=paramDict['show_filters'],
                                     fig='red', field='Iscat',sub=221)
    outDict['filters']['f_Iscat']=f_Iscat&f_Iscat1
    outDict['filters']['f_good']=f_Iscat&f_Iscat1&f_xon ##formerly known as f_intens
    ...
    print('ipm_thresh keeps ', 
          np.nonzero(f_Ipm)[0].shape[0]/outDict['numshots'])


    #corr after slop filter if both true!
        
    if paramDict['slope_filter']:

        nanfilt=~np.isnan(Iscat)&~np.isnan(ipmi)&(f_Ipm)&(f_Iscat1)
        ...
        print('slop_filter keeps ', 
              np.nonzero(f_slope)[0].shape[0]/outDict['numshots'])
    ...
    if paramDict['corr_filter']:
        ...
        if paramDict['slope_filter']:
            nanfilt=~np.isnan(Iscat)&~np.isnan(ipmi)&(f_Ipm)&(f_Iscat1)&f_slope
        else:
            nanfilt=~np.isnan(Iscat)&~np.isnan(ipmi)&(f_Ipm)&(f_Iscat1)
    ...
    if paramDict['show_filters']: 
        ...
        if paramDict['corr_filter']:
            ...
            plt.scatter(ipm1[~in_mask],
                        Isum1[~in_mask],marker='.',color='black',alpha=.05)
...
def TTfilter(paramDict,outDict):
    ...
    f_ttamp=(ttamp>0.0002)&(ttamp<1)  #mono
    ...
    l,r,frac,f_ttfwhm=slice_histogram(ttfwhm,
                                      f_lon&f_good&(ttfwhm>0.0001)&(ttfwhm<800),
