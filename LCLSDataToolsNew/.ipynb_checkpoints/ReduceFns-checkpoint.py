...
def FilterCheck(inDir,exper,run,outDir,paramDict1,varDict,return_dict=False):
    ''' only run the filtering steps of ReduceData to set parameters.  Does not save plots or anything. Only processes one run at a time. returns outDict if you want it'''
    plt.close('all')
    fname=inDir+exper+'_Run%04i.h5'%run
    paramDict=paramDict1.copy()
    print('loading ', fname)
